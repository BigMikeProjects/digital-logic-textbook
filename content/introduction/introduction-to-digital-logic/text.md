...
![Silicon wafer in a cleanroom](./images/silicon-wafer-cleanroom.png)
*A silicon wafer in a cleanroom, where billions of transistors are patterned onto a single chip.*
...
![Digital logic signals on an oscilloscope](./images/oscilloscope-digital-logic-signals.png)
*An oscilloscope shows a digital signal switching between two well-defined voltage levels—the physical manifestation of abstract binary values.*

Transistors are the atomic building blocks of every digital system, but they are rarely the most convenient level at which to think. Our first step, therefore, is to assemble transistors into standard configurations that form logic gates such as AND, OR, and NOT. The logic gate is the abstraction we will rely on most heavily from here on. Each gate implements a Boolean function, producing an output that depends only on its current inputs. At this level, the designer no longer reasons about charge carriers or doping profiles; instead, circuit behavior is captured using truth tables, logic expressions, and timing relationships. The transistors underneath remain essential, but they are hidden behind a clean functional description.
...
![Traffic light as a finite state machine](./images/traffic-light-state-machine.png)
*A traffic light controller is a familiar finite state machine, cycling through red, yellow, and green states in response to timers and sensor inputs.*
...
![Data center and computing performance growth](./images/datacenter-performance-growth.png)
*Modern data centers, where racks of processors built from the same transistor-level foundations deliver computing performance that continues to grow dramatically.*
...
Physical devices and the organized systems that compute sit at opposite ends of a long chain, and the aim here is to make that chain concrete at every link. Once these basics are solid, you can take on harder problems in computer engineering and know exactly where each piece fits.