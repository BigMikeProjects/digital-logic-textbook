...
Take a four-variable function $F(A, B, C, D)$ whose map has zeros that fall into two natural groups when you circle them. One **group of four** holds $A = 0$ and $B = 0$ constant (with $C$ and $D$ varying), giving the product term $\bar{A}\bar{B}$. The other group is the **four corners**, which by edge-wrapping form a single group of four with $B = 0$ and $D = 0$ constant, giving the term $\bar{B}\bar{D}$. (A don't-care cell was pulled into the first group to let it grow to four cells.)
